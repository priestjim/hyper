...
# test: 		    rebar3 as test do eunit -c, cover
# dialyzer:         rebar3 as test do dialyzer
# xref:             rebar3 as dev do xref
# dist:             rebar3 as test do compile, eunit -v -c, xref, dialyzer, cover
...
# Build targets
.PHONY: all test dialyzer xref spec dist perf_report estimate_report
...
shell: $(REBAR) epmd
	@REBAR_PROFILE=dev $(REBAR) do shell --name hyper@127.0.0.1
